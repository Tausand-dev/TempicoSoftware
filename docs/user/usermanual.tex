...
Changes(\includegraphics[width=1.14202in,height=0.13577in]{images/file-path-settings-01.png}),
which updates the configuration according to the entered values;
Cancel(\includegraphics[width=0.9903in,height=0.14645in]{images/file-path-settings-02.png}),
which closes the window without applying any changes; and Default
Settings(\includegraphics[width=1.14724in,height=0.14646in]{images/file-path-settings-03.png}),
...
\subsection{Status bar}\label{status-bar}}

Across every measurement tab in Tempico Software, a status bar keeps the
user informed about the state of the acquisition without requiring the
user to interrupt or inspect the ongoing measurement in detail. In practice,
the Start-stop histogram, Lifetime histograms, Time stamping, Counts
estimation, Autocorrelation (FCS), and g\(^{(2)}\) (HBT) tabs all drive
this bar with a short status text accompanied by a small colored dot,
whose color summarizes the current state: grey (\includegraphics[width=0.14in,height=0.14in]{images/Grey-dot.png}) before the
measurement starts, green (\includegraphics[width=0.14in,height=0.14in]{images/Green-dot.png}) while it is running normally, orange
(\includegraphics[width=0.14in,height=0.14in]{images/Orange-dot.png}) when a warning condition is detected, and yellow
(\includegraphics[width=0.14in,height=0.14in]{images/Yellow-dot.png}) during certain transitional, non-acquiring states. Each of these
is described in more detail below.

\hypertarget{status-bar-idle}{%
\subsubsection{Idle state}\label{status-bar-idle}}

In all six measurement tabs, the status dot is initialized as a grey
circle (\includegraphics[width=0.14in,height=0.14in]{images/Grey-dot.png}) as soon as the tab is built, before any measurement has been
started; this grey dot therefore represents the absence of a running
measurement rather than a state actively reported during acquisition.

\begin{figure}[H]
\centering
\includegraphics[width=5.9in,height=0.65in]{images/idle-state.png}
\label{fig:30}
\end{figure}

\hypertarget{status-bar-active}{%
\subsubsection{Active measurement}\label{status-bar-active}}

Once a channel is producing valid data, the status bar switches to its
green code (\includegraphics[width=0.14in,height=0.14in]{images/Green-dot.png}), and the accompanying text reports the acquisition
progress in a way that matches what each technique measures. For tabs
with continuous measurements, like the start-stop histogram feature,
the status bar simply indicates that the measurement is running,
without a numeric indicator of progress.
...
For tabs that acquire toward a defined target, such as a configured
number of measurements or records, the status bar additionally reports
the completion percentage alongside the running message; any warning
issued while such a target is active is annotated with that same
percentage. Some tabs also support scheduling a start time, in which
case the status bar instead shows a countdown until the configured
start time is reached; while this countdown is running the software
behaves as though the measurement were already underway, and the tab
and device configuration cannot be changed until either the countdown
finishes or the user cancels it with the stop button.
...
When autosave is writing accumulated data to disk during a running
measurement, the status bar switches to the message ``Saving data'',
and once the user stops the measurement, the same processing state
(covered in the Paused and processing states subsection below) reports
the percentage of records already sorted and written to their final
path.

\begin{figure}[H]
\centering
\includegraphics[width=5.9in,height=0.65in]{images/perform-a-measurement-3-05.png}
\label{fig:83}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=5.9in,height=0.65in]{images/perform-a-measurement-3-13.png}
\label{fig:90}
\end{figure}

Some tabs also report their own acquisition counters, such as the
number of calls, events, or the elapsed time, while a measurement is
running; once the configured acquisition duration has elapsed, these
tabs return to this same green state (\includegraphics[width=0.14in,height=0.14in]{images/Green-dot.png}), showing a finishing message
instead of a counter update, once the device settings have been
restored.

\hypertarget{status-bar-missing-pulses}{%
\subsubsection{Missing channel pulses}\label{status-bar-missing-pulses}}

When a required channel stops receiving valid pulses, some tabs switch
the status dot to orange (\includegraphics[width=0.14in,height=0.14in]{images/Orange-dot.png}) and display a status
message. If the start channel itself is not producing pulses, the tabs
display ``No measurements in Start Channel''. If the start channel is
fine but one or more configured stop channels are silent, they display
``No measurements in Stop Channel'' (specifying the affected channel
when applicable, such as ``No measurements in Stop Channel A''). A
possible solution when the start channel shows this warning is to
adjust the Threshold Voltage in General settings, for example setting
it to $1.6\text{ V}$ and checking whether the measurement then proceeds
correctly; if the warning persists after trying different values and
the setup and input signal have been ruled out as the cause, Tausand
support should be contacted.

In the Counts estimation tab specifically, a channel that stops
producing enough pulses is instead flagged directly in the results
table with the message Low Counts, rather than through the shared
colored-dot warning layout described above, reflecting that this tab
reasons about insufficient pulse density on an otherwise active
channel rather than a fully silent start or stop line. Once
measurements are registered again on the channel, the process resumes
normally.

\begin{figure}[H]
\centering
\includegraphics[width=5.9in,height=0.65in]{images/perform-a-measurement-5-04.png}
\label{fig:118}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=5.9in,height=0.65in]{images/perform-a-measurement-4-04.png}
\label{fig:106}
\end{figure}

\hypertarget{status-bar-paused-processing}{%
\subsubsection{Paused and processing states}\label{status-bar-paused-processing}}

The Time stamping and Lifetime histograms tabs share a third status
color, yellow (\includegraphics[width=0.14in,height=0.14in]{images/Yellow-dot.png}), reserved for transitional states in which the device is not
actively acquiring new pulses but the measurement has not finished
either. In the Time stamping tab, this state covers three situations:
while the measurement is paused by the user, the status bar shows
``Paused measurement'', with the completion percentage appended when
the tab is running in sample-size mode; while autosave is writing
accumulated data to disk, it shows ``Processing data''; and once the
user stops the measurement, it shows the same ``Processing data''
message, followed by an explicit percentage, while the recorded
entries are sorted and written to their final path.

\begin{figure}[H]
\centering
\includegraphics[width=5.9in,height=0.65in]{images/perform-a-measurement-3-16.png}
\label{fig:91}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=5.9in,height=0.65in]{images/perform-a-measurement-3-17.png}
\label{fig:92}
\end{figure}

In the Lifetime histograms tab, this same third color is used only
once, at the very end of the acquisition loop, when the status bar
displays ``Ending measurement'' while the thread finalizes the run
before the measurement is marked as complete.

\hypertarget{status-bar-module-specific-warnings}{%
\subsubsection{Module-specific configuration
warnings}\label{status-bar-module-specific-warnings}}

Two of the tabs raise an additional warning, shown with the same orange
code (\includegraphics[width=0.14in,height=0.14in]{images/Orange-dot.png}) used for missing pulses, that is tied to a configuration
check particular to that functionality rather than to a silent channel.
In the Start-stop histogram tab, when a channel repeatedly produces
out-of-range values and the user has been prompted to change that
channel's mode without doing so, the status bar replaces the running
message with ``Consider changing mode of the channels: '', followed by
the letter of every affected channel; this message is exclusive to the
Start-stop histogram tab and does not appear in any other module. In
the Counts estimation tab, before the actual measurement begins, the
status bar displays ``Estimating number stops in channel'', followed
by the channel letter and a completion percentage, while the software
automatically determines how many stops each channel can reliably
register; this calibration message is likewise exclusive to Counts
estimation and precedes, rather than replaces, the Low Counts
reporting described above.

\begin{figure}[H]
\centering
\includegraphics[width=5.9in,height=0.65in]{images/settings-before-measurement-02.png}
\label{fig:54-status}
\end{figure}
...
The progress of the measurement is reported by the status bar described
in Section~\ref{status-bar}; if any channel is not receiving data, that
same bar will display a message indicating so.
...
each channel independently (\includegraphics[width=0.52823in,height=0.16368in]{images/perform-a-measurement-10.png},
...
to begin the measurement. As with all measurement tabs, progress and any
channel warnings are reported through the status bar described in
Section~\ref{status-bar}, including the completion percentage toward the
configured number of measurements.
...
the counting process, reporting the calibration progress through the
status bar as described in Section~\ref{status-bar}.
...
results table will display the Low Counts message described in
Section~\ref{status-bar}.
...
selected tab
(\includegraphics[width=0.7in,height=0.14894in]{images/perform-a-measurement-3-01.png},\includegraphics[width=2.68832in,height=0.14889in]{images/perform-a-measurement-3-02.png}).
...
before the measurement starts, as described in Section~\ref{status-bar}.
...
(\includegraphics[width=2.0in,height=0.1in]{images/perform-a-measurement-3-14.png}, \includegraphics[width=0.76in,height=0.14in]{images/perform-a-measurement-3-15.png})
...
without restarting the entire process. While paused, or while data is
being processed at the end of the run, the status bar reflects this in
the transitional state described in Section~\ref{status-bar}.
...
Continuous measurement option are locked and cannot be changed. The
...
values: Calls, which tracks the total requests sent to the device since the measurement started; Events, which indicates the number of
...
total configured duration. Alongside this panel, the status bar reports
the acquisition state as described in Section~\ref{status-bar}; if a
warning persists, the user should check the corresponding cable
connections and the signal levels before continuing the measurement.
...
\[G(\infty) + \frac{1}{N} \cdot \left( 1 + \frac{\tau}{\tau_{D}} \right)^{- 1} \cdot \left( 1 + a^{- 2}\frac{\tau}{\tau_{D}} \right)^{- \frac{1}{2}}\]
...
number of particles in the observation volume; $\tau_D$, the diffusion time, in ms; and $a$, the structure parameter, defined as the ratio between the axial and lateral radii of the observation volume.

\hypertarget{anomalous-diffusion}{%
\subparagraph{Anomalous diffusion}\label{anomalous-diffusion}}

\[G(\infty) + \frac{1}{N} \cdot \left( 1 + \left( \frac{\tau}{\tau_{D}} \right)^{\alpha} \right)^{- 1} \cdot \left( 1 + a^{- 2}\left( \frac{\tau}{\tau_{D}} \right)^{\alpha} \right)^{- \frac{1}{2}}\]

Extends the 3D Gaussian diffusion model to account for non-Brownian,
anomalous transport. In addition to N, $\tau$\textsubscript{D} (ms), and $a$,
...
\[\ \ \ \ G(\infty) + \frac{1}{N} \cdot K \cdot \exp\left\lbrack - \frac{\tau}{\tau_{B}} \right\rbrack\ \ \ \ \ \ \ \ \]
...
k\textsubscript{on}/k\textsubscript{off}, and relates to the value of the curve at $\tau=0$ through $G(0) = \frac{K}{N}$.
...
\[G(\infty) + \frac{1}{N} \cdot \left( {\alpha_{1} \cdot \left( 1 + \frac{\tau}{\tau_{D1}} \right)}^{- 1} \cdot \left( 1 + a^{- 2}\frac{\tau}{\tau_{D1}} \right)^{- \frac{1}{2}} + {\alpha_{2} \cdot \left( 1 + \frac{\tau}{\tau_{D2}} \right)}^{- 1} \cdot \left( 1 + a^{- 2}\frac{\tau}{\tau_{D2}} \right)^{- \frac{1}{2}} \right)\]
...
$\alpha$\textsubscript{1}, the amplitude fraction of the second species $\alpha$\textsubscript{2}, calculated as $\alpha$\textsubscript{2}$=1-$$\alpha$\textsubscript{1},  and the structure parameter $a$.
...
configured from 10 ns up to 100,000 ns; together with the bin width, this
...
before starting the measurement, so a rate, expressed in
...
also shows the estimated rate, in counts per second. Alongside this
panel, the status bar reports the acquisition state as described in
Section~\ref{status-bar}; if a warning persists, the user should check
the corresponding cable connections and the signal levels before
...
with a bunching shoulder. The fitted parameters are $a$, the relative
