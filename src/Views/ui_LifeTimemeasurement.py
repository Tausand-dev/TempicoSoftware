...
class UiLifeTime(object):
    """
    UI definition for the Lifetime measurement tab.

    This class was generated by the Qt Designer UI compiler from
    'LifeTimemeasurementLNLkHv.ui' and later extended by hand with extra
    widgets and helper methods (fit function selector, parameters table,
    bin width/time range logic, and the status color indicator) that are
    not part of the auto-generated layout.

    :param object: Base class placeholder used by the Qt UI compiler.
    """
    def setupUi(self, Form):
        """
        Builds and arranges every widget of the Lifetime measurement tab.

        Most of this method is boilerplate produced by the Qt Designer
        compiler (creating frames, layouts, labels, combo boxes, etc. and
        wiring their size policies). On top of that, it adds the manual
        parts specific to this tab: the bin width/number of bins/maximum
        time combo boxes, the fit function selector and its parameters
        table, and the connections that link user input to the custom
        calculation methods below (setTimeRange, maxNumberBins, etc).
    ...
    def retranslateUi(self, Form):
        """
        Sets the visible text of every widget (labels, buttons, combo box
        items, table headers) created in setupUi, plus the initial values
        of the fit-parameters table.

        Standard Qt Designer boilerplate; the fit parameters table entries
        (I0, tau0, R^2) are the manual addition, initialized as "Undefined"
        until a measurement is fitted.
    ...
    def drawColorPoint(self):
        """
        Draws a small gray circle on drawPointLabel as the initial
        (idle) status indicator, before any measurement colors it.
    ...
    def set_latex_to_label(self, text):
        """
        Displays the given (HTML-formatted) fit equation on equationLabel.

        Qt labels support a subset of HTML, so superscripts/subscripts in
        the equation (e.g. "I0 e^(-t/tau0)") are passed in as HTML tags
        rather than real LaTeX.

        :param text: HTML string with the equation to display.
        :return: None
        """
    ...
    def functionChange(self):
        """
        Updates the equation label and the fit parameters table when the
        user selects a different fit function.

        Each fit function (Exponential, Kohlrausch, Shifted exponential,
        Double exponential) has its own set of parameters, so the table
        row count and row labels are rebuilt to match the chosen function
        before the row heights are reapplied.
    ...
    def startStopChannelChange(self):
        """
        Recomputes and displays the maximum time range when the start
        channel selection changes.

        If the start channel is set to the placeholder ("Start channel",
        index 0) the device enforces a minimum range of 24000 ps, so the
        number of bins is adjusted accordingly via minimumNumberBins.
    ...
    def setTimeRange(self):
        """
        Recalculates and displays the maximum time range when the number
        of bins is changed by the user.

        Skips the recalculation once via `sentinelChangeBins` right after
        maxNumberBins() repopulates the combo box, to avoid overwriting
        the range it already set through setTimeRangeChangeBinWidth.
    ...
    def setTimeRangeChangeBinWidth(self):
        """
        Recalculates and displays the maximum time range using the
        smallest available number of bins, right after the bin width
        changes and the bins combo box has just been repopulated.
    ...
    def maxNumberBins(self):
        """
        Rebuilds the number-of-bins combo box when the bin width changes,
        keeping only the values whose total range (bins * bin width)
        does not exceed the device's maximum range of 4e9 ps.
    ...
    def transformUnits(self, unitsValue):
        """
        Converts a "<number> <unit>" bin-width string (e.g. "4 ns") into
        its equivalent numeric value in picoseconds.

        :param unitsValue: Bin width as text, e.g. "480 ps", "2 ns", "1 µs".
        :return: The value expressed in picoseconds (float).
        """
    ...
    def transformIntoString(self,value):
        """
        Picks the most convenient display unit for a value given in
        picoseconds, along with the divisor needed to convert to it.

        :param value: A time value expressed in picoseconds.
        :return: A two-item list [unit_label, divisor], e.g. ["ns", 1000].
        """
        if value < 1e3:
            return ["ps",1]
        elif value < 1e6:
    ...
    def minimumNumberBins(self,minimum):
        """
        Repopulates the number-of-bins combo box so it only offers values
        greater than or equal to the given minimum, used when the current
        selection would otherwise fall under the device's minimum range.

        :param minimum: The smallest allowed number of bins.
        :return: None
        """
        self.numberBinsComboBox.clear()
